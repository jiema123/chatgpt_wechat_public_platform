...
func NewUserService(cache *cache.Cache, userId string) UserServiceInterface {
	return &UserService{
		cache:  cache,
		userId: userId,
	}
...
func (s *UserService) SetUserSessionContext(question, reply string) {
	value := question + "\n" + reply
	s.cache.Set(s.userId, value, time.Second*config.LoadConfig().SessionTimeout)
}
